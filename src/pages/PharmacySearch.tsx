...
import { Search, SlidersHorizontal, Store, ShoppingCart, ChevronLeft, ChevronRight, Info } from "lucide-react";
...
export default function PharmacySearch() {
  ...
  const categories = ["الكل", "مسكنات الألم", "فيتامينات", "العناية بالبشرة", "أدوات طبية"];
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="flex-1 space-y-6">
              ...
              {filteredMeds.map((med) => {
                const availablePharmacies = pharmacies.filter((p) => p.medicines.some((m) => m.id === med.id));
                return (
                    ...
                    <div className="flex flex-col sm:flex-row">
                      ...
                      <div className="p-6 flex-1 text-center sm:text-right">
                        ...
                        <div className="mt-3 inline-block bg-accent rounded-lg px-3 py-1.5">
                          ...
                          <p className="text-lg font-bold text-primary">{formatPrice(med.price)}</p>
                        </div>
                      ...
                      <div className="p-6 border-t sm:border-t-0 sm:border-r border-border flex-1">
                        ...
                        <div className="space-y-3">
                          {availablePharmacies.slice(0, 3).map((p) => (
                            <Link key={p.id} to={`/pharmacies/${p.id}`} className="flex items-center gap-3 p-3 rounded-lg border border-border hover:border-primary transition-colors">
                              ...
                              <div className="text-left">
                                ...
                                <p className="font-bold text-sm">{formatDistance(p.distance)}</p>
                              </div>
                              <div className="text-left">
                                <span className="text-xs text-muted-foreground">السعر</span>
                                <p className="font-bold text-sm text-primary">{formatPrice(med.price)}</p>
                              </div>
                              <span className={`text-xs px-2 py-0.5 rounded-full ${p.isOpen ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"}`}>
                                {p.isOpen ? "متوفر حالياً" : "غير متوفر"}
                              </span>
                            </Link>
                          ))}
                        </div>
